...
const SupplierDashboard = () => {
  ...
  const [submittingCounterResponse, setSubmittingCounterResponse] =
    useState(false);
        ...
        const totalEarnings = offers
          ...
          .reduce(
            (sum, offer) =>
              sum +
              offer.offeredPrice *
                parseFloat(offer.request?.quantity?.match(/\d+/)?.[0] || 0),
            0
          );
        const responseRate =
          offers.length > 0
  ...
  const fetchOpenRequests = async () => {
    try {
      ...
      const res = await axios.get(`${backendURL}/api/offers/browse-requests`, {
        ...getAuthHeaders(),
  ...
  const fetchGroupRequests = async () => {
    try {
      ...
      const res = await axios.get(`${backendURL}/api/groups/requests/active`, {
        ...getAuthHeaders(),
        params: searchQuery ? { item: searchQuery } : {},
      });
  ...
  const submitCounterResponse = async () => {
    if (
      ...
      (!counterResponseData.offeredPrice || !counterResponseData.eta)
    ) {
      toast({
        title: "Error",
  ...
  const filteredOpenRequests = openRequests.filter(
    (request) =>
  ...
  const filteredGroupRequests = groupRequests.filter(
    (group) =>
  ...
  const filteredMyOffers = [
    ...myOffers,
    ...myGroupOffers.map((offer) => ({ ...offer, isGroupOffer: true })),
  ].filter(
    (offer) =>
      offer.request?.item?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (
        offer.request?.vendor?.name ||
        offer.groupRequest?.group?.leader?.name ||
        ""
      )
        .toLowerCase()
  ...
  return (
    <>
          ...
          >
            ...
            <TabsContent value="individual" className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold">
                  Browse Vendor Requests
                </h2>
                <Badge variant="outline">
                  {filteredOpenRequests.length} active requests
                </Badge>
              </div>
            ...
            <TabsContent value="groups" className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant="outline">
                  {filteredGroupRequests.length} active groups
                </Badge>
              </div>
            ...
            <TabsContent value="my-offers" className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant="outline">
                  {filteredMyOffers.length} offers
                </Badge>
              </div>
                        ...
                        <CardContent className="pt-0">
                            ...
                            <div className="mb-4">
                              ...
                              <div className="flex gap-2 flex-wrap">
                                ...
                                {offer.deliveryOptions.canDeliver && (
                                  <Badge variant="outline" className="text-xs">
                                    Delivery (₹
                                    {offer.deliveryOptions.deliveryCharge})
                                  </Badge>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="flex items-center justify-between">
                              ...
                              {offer.status === "countered" && (
                                <div className="flex gap-2">
                                  <Button
                                    size="sm"
                                    onClick={() =>
                                      handleCounterResponse(offer, "accept")
                                    }
                                  >
                                    Accept Counter
                                  </Button>
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      handleCounterResponse(offer, "counter")
                                    }
                                  >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() => {
                                const requestId = isGroupOffer
                                  ? request?._id || request?.id
                                  : request?._id || request?.id;

                                if (requestId) {
                                  if (isGroupOffer) {
                                    // Navigate to group request details
                                    navigate(
                                      `/vendor/group-requests/${requestId}`
                                    );
                                  } else {
                            ...
                            >
                              ...
                              View {isGroupOffer ? "Group Request" : "Request"}
                            </Button>
                          </div>
                        </CardContent>
                      </Card>
      ...
      <Dialog
        open={counterResponseDialog}
        onOpenChange={setCounterResponseDialog}
      >
